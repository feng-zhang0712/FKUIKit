...
final class KeyboardAvoidanceExampleViewController: FKPresentationExamplePageViewController {
  ...
  override func viewDidLoad() {
    ...
    addPrimaryButton(title: "Present form") { [weak self] in
      ...
      config.sheet.detents = [.fraction(0.55), .full]
      config.keyboardAvoidance.isEnabled = true
      config.keyboardAvoidance.strategy = (self.strategyIndex == 0) ? .adjustContainer : .adjustContentInsets
