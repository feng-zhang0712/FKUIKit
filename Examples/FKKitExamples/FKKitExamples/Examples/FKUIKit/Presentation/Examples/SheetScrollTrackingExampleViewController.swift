...
final class SheetScrollTrackingExampleViewController: FKPresentationExamplePageViewController {
  ...
  override func viewDidLoad() {
    ...
    addPrimaryButton(title: "Present table") { [weak self] in
      ...
      config.sheet.detents = [.fraction(0.35), .full]

      switch self.trackingIndex {
      case 0:
        config.sheet.scrollTrackingStrategy = .explicit(FKWeakReference(tableVC.tableView))
      case 1:
        config.sheet.scrollTrackingStrategy = .automatic
      default:
        config.sheet.scrollTrackingStrategy = .disabled
      }
