...
final class BlurBackdropExampleViewController: FKPresentationExamplePageViewController {
  ...
  override func viewDidLoad() {
    ...
    addPrimaryButton(title: "Present") { [weak self] in
      ...
      config.backdrop.style = .blur(effect: style, alpha: CGFloat(self.alpha), vibrancy: nil)
      _ = FKPresentationExampleHelpers.present(from: self, title: "Blur backdrop", configuration: config)
    }
  }
}
