  ...
  override func viewDidLoad() {
    ...
    addPrimaryButton(title: "Present (auto choose)") { [weak self] in
      ...
      if UIAccessibility.isReduceMotionEnabled {
        config.animation.preset = .fade
        config.animation.duration = 0.18
      } else {
        config.animation.preset = .systemLike
      }

      _ = FKPresentationExampleHelpers.present(from: self, title: "Reduce Motion aware", configuration: config)
    }
