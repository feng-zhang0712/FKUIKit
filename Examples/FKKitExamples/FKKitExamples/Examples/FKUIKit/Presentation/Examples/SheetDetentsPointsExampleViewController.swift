...
final class SheetDetentsPointsExampleViewController: FKPresentationExamplePageViewController {
  ...
  override func viewDidLoad() {
    ...
    addPrimaryButton(title: "Present") { [weak self] in
      ...
      config.sheet.detents = [.fixed(240), .fixed(520)]
      config.sheet.initialDetentIndex = 0
      self.currentController = FKPresentationExampleHelpers.present(from: self, title: "Points detents", configuration: config)
    }
