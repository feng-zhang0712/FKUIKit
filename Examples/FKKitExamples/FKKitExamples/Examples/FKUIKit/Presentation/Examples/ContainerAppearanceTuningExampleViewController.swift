...
final class ContainerAppearanceTuningExampleViewController: FKPresentationExamplePageViewController {
  ...
  override func viewDidLoad() {
    ...
    addPrimaryButton(title: "Present") { [weak self] in
      ...
      config.cornerRadius = CGFloat(self.cornerRadius)
      config.shadow.opacity = self.shadowOpacity
      config.shadow.radius = CGFloat(self.shadowRadius)
      config.border.isEnabled = self.borderEnabled
      config.border.width = CGFloat(self.borderWidth)

      _ = FKPresentationExampleHelpers.present(from: self, title: "Appearance", configuration: config)
    }
  }
}
