...
final class SheetDetentsFractionExampleViewController: FKPresentationExamplePageViewController {
  ...
  override func viewDidLoad() {
    ...
    addPrimaryButton(title: "Present") { [weak self] in
      ...
      config.sheet.detents = [.fraction(CGFloat(self.smallFraction)), .fraction(CGFloat(self.largeFraction))]
      config.sheet.initialDetentIndex = 0
      _ = FKPresentationExampleHelpers.present(from: self, title: "Fraction detents", configuration: config)
    }
  }
}
