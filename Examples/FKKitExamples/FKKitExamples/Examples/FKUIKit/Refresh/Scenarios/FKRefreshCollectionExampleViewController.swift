...
final class FKRefreshCollectionDemoViewController: UIViewController {
  ...
  private lazy var outcomeControl: UISegmentedControl = {
    ...
    control.addTarget(self, action: #selector(outcomeSegmentChanged), for: .valueChanged)
    return control
  }()
  ...
  override func viewDidLoad() {
    ...
    collectionView.fk_addPullToRefresh(configuration: cfg) { [weak self] in
      FKRefreshExampleCommon.simulateRequest(delay: 1.0) {
        self?.items = (0..<24).map { "Cell \($0)" }
        self?.collectionView.reloadData()
        self?.collectionView.fk_pullToRefresh?.endRefreshing()
        self?.collectionView.fk_loadMore?.resetFooterAfterPullToRefresh()
      }
    }
    ...
    collectionView.fk_addLoadMore(configuration: cfg) { [weak self] in
      FKRefreshExampleCommon.simulateRequest(delay: 0.9) {
        ...
        self.items.append(contentsOf: (n..<(n + 8)).map { "Cell \($0)" })
        self.collectionView.reloadData()
  ...
  @objc private func outcomeSegmentChanged() {
    collectionView.fk_pullToRefresh?.cancelCurrentAction(resetState: true)
    collectionView.fk_loadMore?.cancelCurrentAction(resetState: true)
    items = (0..<24).map { "Cell \($0)" }
    collectionView.reloadData()
    collectionView.setContentOffset(.zero, animated: false)
    collectionView.fk_loadMore?.resetFooterAfterPullToRefresh()
  }
