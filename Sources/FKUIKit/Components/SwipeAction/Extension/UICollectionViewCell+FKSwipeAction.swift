//
// UICollectionViewCell+FKSwipeAction.swift
//
// Convenience APIs for attaching swipe actions to collection cells.
//

import UIKit

extension UICollectionViewCell: FKSwipeActionCellHost {}

public extension UICollectionViewCell {
  /// Configures swipe actions for this cell.
  ///
  /// Call this in `collectionView(_:cellForItemAt:)` for reuse-safe updates.
  ///
  ...
  func fk_configureSwipeAction(_ configuration: FKSwipeActionConfiguration = FKSwipeAction.defaultConfiguration) {
    ...
    if configuration.behavior.closesOnScroll, let collectionView = fk_enclosingScrollView as? UICollectionView {
      // Install auto-close observer once per collection view when enabled.
      collectionView.fk_enableSwipeActionAutoCloseOnScroll()
    }
  }
