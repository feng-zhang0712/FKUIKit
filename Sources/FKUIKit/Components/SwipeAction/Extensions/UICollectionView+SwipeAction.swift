...
private enum FKSwipeCollectionAssociationKey {
...
public extension UICollectionView {
  ...
  ///   - provider: Per-item configuration provider. Called with the current `IndexPath` to supply
  ...
  func fk_enableSwipeActions(
    ...
    runOnMain { [weak self] in
      ...
      objc_setAssociatedObject(
        self,
        &FKSwipeCollectionAssociationKey.provider,
      ...
      manager.setProvider { [weak self] indexPath in
        guard
          let self,
          let box = objc_getAssociatedObject(self, &FKSwipeCollectionAssociationKey.provider) as? FKSwipeActionProviderBox
        else {
  ...
  /// Manager instance associated with this collection view.
  internal var fk_swipeActionManager: FKSwipeActionManager {
    if let cached = objc_getAssociatedObject(self, &FKSwipeCollectionAssociationKey.manager) as? FKSwipeActionManager {
      return cached
    }
    ...
    objc_setAssociatedObject(self, &FKSwipeCollectionAssociationKey.manager, manager, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
    return manager
  }
